<template>
  <section class="lecture" id="l8" ref="rootEl">
    ...
    <p>The <strong>RSSM</strong> (Recurrent State-Space Model) at Dreamer's heart maintains a state with two halves: a <em>deterministic</em> recurrent path \(h_t = f(h_{t-1}, z_{t-1}, a_{t-1})\) (memory; stable gradient highway) and a <em>stochastic</em> latent \(z_t\) (uncertainty; sampled from a prior \(p(z_t|h_t)\) when imagining, from a posterior \(q(z_t|h_t,o_t)\) when observing). The swap is the key move: the posterior \(q(z_t|h_t,o_t)\) runs while real observations stream in — it gets to peek at \(o_t\); the prior \(p(z_t|h_t)\) takes over during imagination, when there is nothing to peek at.</p>
