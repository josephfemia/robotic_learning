<template>
  <section class="lecture" id="l7" ref="rootEl">
    ...
    <p>Concretely: a three-step episode with rewards \(r=(1,0,2)\) has returns-to-go \(\hat R=(3,2,2)\), so the training stream reads \((3,s_0,a_0,\;2,s_1,a_1,\;2,s_2,a_2)\) — each action preceded by the return still owed. At test time you write the first number yourself: prompt 3 and the model emits the actions that, in the data, tended to follow a promise of 3.</p>
    <p>No value function, no Bellman backup, no policy gradient — supervised learning plus a clever prompt. It matches offline-RL baselines on standard benchmarks.</p>
