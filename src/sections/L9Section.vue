<template>
  <section class="lecture" id="l9" ref="rootEl">
    ...
    <h4>Manufacture supervision from one aimless trajectory</h4>
    <p>How does an hour of aimless, unlabeled play become thousands of demonstrations without a single human label? Pick any window of the stream below: whatever the hand happened to be doing, that window is — already, exactly — a demonstration of reaching its own final state. Slide the window and watch the dataset assemble itself from nothing, then harvest every window at once and compare the haul against scripting demos one at a time.</p>
    <RelabelWidget />

    <h3><span class="knum">9.3</span>The VLA paradigm: RT-1 → RT-2</h3>
    <p><strong>RT-1</strong> proved the scaling recipe on one embodiment: ~130k real demonstrations, 700+ tasks, a transformer over image tokens + language embedding, actions discretized to 256 bins per dimension (L7's tokenization choice) — 97% success on seen tasks and meaningful generalization to new combinations. <strong>RT-2</strong> then made the move that named the field: start from a pretrained <em>vision-language model</em> (web-scale knowledge of objects, relations, text) and <strong>fine-tune it to emit action tokens as if they were words</strong> — co-training on web data and robot data so the language abilities don't wash out. The payoff is <em>semantic</em> generalization no amount of robot-only data buys: "pick up the extinct animal" → the dinosaur toy; "move the can to Taylor Swift" → it knows Taylor Swift. The robot's grounding problem meets the internet's knowledge, in one set of weights. That is the Vision-Language-<em>Action</em> model: a VLM whose output vocabulary includes motor control. RT-2's in-house lineage continues in <strong>Gemini Robotics 1.5</strong> (Sept 2025): a multi-embodiment VLA that interleaves explicit reasoning with acting, paired with a dedicated embodied-reasoning model, and transfers motions across bodies as different as ALOHA, a Franka arm, and the Apollo humanoid.</p>

    <div class="callout"><span class="co-label">The VLA recipe, step by step</span>
    <strong>1.</strong> Start from a <em>pretrained</em> vision-language model (web-scale knowledge of objects, relations, language). <strong>2.</strong> Extend its vocabulary/output with <em>actions</em> — either as discrete tokens (RT-2: bin each dimension into words) or via a continuous head (Octo/π0: a diffusion or flow expert generating chunks). <strong>3.</strong> <em>Co-train</em> on web data + robot trajectories so the language/vision abilities don't wash out as it learns control (this co-training is the load-bearing trick). <strong>4.</strong> At test time, prompt with an instruction + current image; the model emits action tokens, which are detokenized into motor commands. <strong>5.</strong> Adapt cheaply to a new robot/task with a little data via LoRA fine-tuning (LoRA: freeze the pretrained weights and train tiny low-rank correction matrices inserted into each layer — fine-tuning at a small fraction of the parameters and memory). Every model in the table below is a different choice at steps 1–2.</div>

    <h3><span class="knum">9.4</span>Open data, open models: OXE, Octo, OpenVLA</h3>
    <p><strong>Open X-Embodiment</strong> pooled trajectories from 20+ institutions' different robots into one corpus (over a million trajectories, 22 embodiments) and showed the unreasonable result: co-training across <em>other people's robots</em> improves <em>your</em> robot — positive cross-embodiment transfer, the field's ImageNet moment for data. <strong>Octo</strong> built the open generalist on it: a transformer trunk with a <strong>diffusion action head</strong> (L6 arrives in the generalist stack — continuous, multimodal chunks instead of binned tokens; the field has since largely converged on <em>flow-matching</em> heads for the same reason §6.4 gives — fewer sampling steps at control rate) and modular goal interfaces. <strong>OpenVLA</strong> — with Mees among the authors — is the open RT-2: a 7B VLA (Llama-2 language backbone; fused DINOv2 + SigLIP vision features) trained on ~970k OXE trajectories (a filtered training subset of the 1M+ corpus — Octo uses a different ~800k subset; the differing counts reflect curation choices, not contradictions), outperforming the 55B RT-2-X while being fully open, and demonstrating cheap downstream adaptation via LoRA fine-tuning on consumer GPUs. The strategic meaning: the VLA recipe stopped being a closed-lab capability and became infrastructure anyone (including this course's projects) can build on — a lineage NVIDIA's open GR00T N1 → N2 humanoid models now extend, with N2 jointly predicting actions <em>and</em> next observations, L8's world-model thesis inside a VLA.</p>
    ...
    <p>Physical Intelligence's series is the current reference point, and each step answers a question this course has equipped you to ask. <strong>π0:</strong> how do you get VLM semantics <em>and</em> dexterous 50 Hz control? A PaliGemma VLM backbone with a separate <strong>flow-matching action expert</strong> (L6's §6.4, verbatim) generating continuous action chunks — trained over many embodiments, post-trained per application (laundry folding, table bussing).</p>
    <p><strong><a href="https://arxiv.org/abs/2504.16054" target="_blank" rel="noopener">π0.5</a>:</strong> does it generalize <em>out of the lab</em>? Co-training on heterogeneous data (multi-robot, web, verbal instructions) plus a hierarchical scheme — the model first infers a high-level subtask in language, then decodes actions — yielding cleaning behavior in entirely unseen homes.</p>
    <p><strong>π*0.6 (the week's paper):</strong> can a VLA <em>improve from its own experience</em> instead of plateauing at demonstration quality — Lecture 3's ceiling, at foundation scale? The RECAP recipe: train a value function on mixed-quality experience; label each past action with its advantage (L5) and feed that label as a conditioning token — Decision-Transformer DNA, with advantage in place of return-to-go (L7); at test time, condition on "high advantage" to elicit better-than-average behavior; add expert corrections. Result: roughly doubled throughput and halved failures on long-horizon real tasks (espresso making, box folding), with RL-from-experience as the engine. The arc of the whole course — IL's ceiling, RL's improvement, sequence-model conditioning — converging in one production system. <strong>π0.7 (April 2026)</strong> pushes the arc one step further: a steerable generalist showing the first signs of <em>compositional</em> generalization — recombining trained skills under plain-language coaching in unseen kitchens — and doing zero-shot what π*0.6 needed experience-RL to reach, e.g. espresso-machine operation out of the box.</p>
    ...
    <div class="cmp-scroll"><table class="cmp">
      ...
      <tr><td>π0.7 (2026)</td><td>π0.5 lineage</td><td>Flow-matching expert</td><td>+ plain-language coaching</td><td>First signs of compositional generalization; zero-shot matches experience-RL specialists</td></tr>
    </table></div>
...
import RelabelWidget from '../widgets/RelabelWidget.vue';
