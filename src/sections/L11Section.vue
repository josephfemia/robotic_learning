<template>
  <section class="lecture" id="l11" ref="rootEl">
    ...
    <p><strong>The humanoid question.</strong> The form-factor bet of the decade: maximal generality and maximal difficulty — is it the right hill, or are wheeled platforms + arms the deployable 90%? The bull case: a human-shaped robot inherits a world built for human bodies, and its data problem aligns with the largest corpus we have (human video, L7–L8) with minimal embodiment gap. The bear case: every other item in this ledger — reliability, contact, evaluation — gets strictly harder on a platform that must also not fall over. The lecture won't answer; your portfolio-thinking applies.</p>
