<template>
  <section class="lecture" id="l4" ref="rootEl">
    ...
    <div class="callout miscon"><span class="co-label">Watch out for</span>
      <ul>
        <li>A learned value function (L5 will name it the "critic") doesn't <em>pick</em> actions — it evaluates them. The policy/argmax acts.</li>
    ...
    <p>The target \(r_t + \gamma V(s_{t+1})\) is the Bellman equation with the expectation replaced by one sample <em>and</em> \(V^\pi\) replaced by the current estimate. Using your own estimate inside your own target is called <strong>bootstrapping</strong>. It slashes variance (only one step of randomness) and works online — at the cost of bias while \(V\) is wrong. The MC↔TD axis is a pure <strong>bias–variance tradeoff</strong> — the next lab names the points in between.</p>
...
for episode in range(50_000):
    ...
    <p>You just watched greed freeze a whole corridor of the maze. The cleanest sandbox for that dilemma is a multi-armed bandit: several actions with hidden payoffs, and only your own pulls to learn from. Try the three classic strategies and watch the regret (lost reward vs. always picking the best arm) accumulate.</p>
    <BanditWidget />
    ...
    <p>The claim that the triad is dangerous <em>when all three legs are present</em> is the kind of thing you should verify, not take on faith. Below is a miniature value-learning problem with three switches: <strong>function approximation</strong> (shared features across states), <strong>bootstrapping</strong> (targets built from current estimates), and <strong>off-policy</strong> data. Flip them on one at a time and the value estimates stay bounded. Flip on all three and watch them spiral to infinity: pure self-reference doing the inflating — no reward ever drives it. Remove any single leg and — in this construction — the divergence dies.</p>
    <TriadWidget />
