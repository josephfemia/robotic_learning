<template>
  <section class="lecture" id="l6" ref="rootEl">
    ...
    <p><strong>Rung 3 — Implicit, energy-based policies (IBC).</strong> Learn a scalar energy \(E_\theta(o,a)\); define the policy implicitly: \(\pi(o) = \arg\min_a E_\theta(o,a)\). An energy is an unnormalized negative log-probability: define \(p(a|o) = e^{-E_\theta(o,a)}/Z(o)\), so low energy ⇔ plausible action, and the policy's argmin is the distribution's mode. The catch is the denominator: the partition function \(Z(o) = \int e^{-E_\theta(o,a)}\,da\) is intractable. Train contrastively (InfoNCE — which pushes the true action's energy down and a batch of sampled "negative" actions' energy up, shaping \(E\) without ever computing the normalizer \(Z\)): push the demo action's energy below sampled negatives'. Arbitrarily expressive — including representing <em>discontinuous</em> policies (snap decisions at thresholds), which smooth regressors fundamentally can't — but inference is an inner optimization, and training needs careful negative sampling because \(Z(o)\) stays out of reach (your "normalizing constants are the enemy" instinct from probabilistic ML applies verbatim).</p>
    <p><strong>Rung 4 — Conditional VAEs.</strong> Encode action (sequences) into a latent \(z\) with the standard ELBO; decode conditioned on observation. (Never met a VAE? Two networks: an encoder \(q(z|x)\) compresses a datapoint into a small random code \(z\); a decoder \(p(x|z)\) reconstructs it. Maximizing \(\log p(x)\) directly is intractable — it hides an integral over every possible \(z\) — so you maximize the ELBO (Evidence Lower BOund), a tractable stand-in: reconstruction log-likelihood minus a KL penalty keeping codes near a standard Gaussian. Reconstruct well ⇒ \(z\) captures the data; codes near Gaussian ⇒ you can sample fresh \(z\) at test time. A conditional VAE (CVAE) feeds an extra input — here the observation — to both networks.) Multimodality lives in \(z\). If you've met VAEs, this is exactly that machinery; its robot career peaks as the backbone of ACT in Lecture 7.</p>

    <h3><span class="knum">6.3</span>Diffusion policies: the current workhorse</h3>
    <p>Rungs 1–4 each changed what the network <em>outputs</em>. Diffusion changes what the network <em>does</em>. Recall the two islands of valid demo actions from Lecture 3 — swerve left, swerve right. Instead of emitting an action, suppose the network emitted a <em>direction</em>: given any candidate action, the arrow \(\nabla_a \log p(a\mid o)\) toward more-plausible ones. Sampling would then be easy — drop a point anywhere in action space, follow the arrows uphill, land in a mode; different starting points land in different islands, and multimodality comes for free.</p>
    <p>Two obstacles: far from the data the arrows are undefined, and we'd need to learn them everywhere. Noise fixes both at once. Blur the demo distribution a lot and it becomes one broad hill covering the whole space — arrows defined everywhere, easy to learn; blur it a little and it's nearly the true lumpy landscape. So learn the arrows at every blur level and sample by walking uphill while the blur anneals away. The miracle is what "learn the arrows" costs: at blur level \(k\), the arrow pointing back toward the data is (up to scale) just the noise you mixed in — so training is a regression: noise an action yourself, ask the network to name the noise.</p>

    <h4>From noise to a decision — and back to multimodality</h4>
    <p>Watch that plan run before we write it down. Press <strong>Denoise</strong>: a cloud of pure noise follows the learned arrows uphill as the blur anneals away, resolving into the two valid action modes — "go left" and "go right," the same maneuvers the averaging trap destroyed back in Lecture 3.</p>
    <DiffWidget />
    ...
    <ForwardWidget />
    <p>And "learn the arrows at every blur level" is one network, \(\varepsilon_\theta\), with the (simplified) loss exactly the regression promised above:</p>
    ...
    <p>Sampling runs the reverse: start from pure noise and iteratively denoise — the annealed uphill walk of the opening paragraphs, with \(\varepsilon_\theta\) supplying the arrows, since it is proportional to the <em>score</em> of the <em>noised</em> distribution: \(\nabla_{x}\log p_k(x) \approx -\varepsilon_\theta/\sqrt{1-\bar\alpha_k}\). Note the word carefully: here <em>score</em> means \(\nabla_x \log p\) — the gradient with respect to the sample \(x\), the Stein sense — not L5's score function \(\nabla_\theta \log \pi\), which is the gradient with respect to policy <em>parameters</em> \(\theta\). Same word, different variable. And the picture "fixed log-density, climb the gradient" is a useful simplification, not the exact dynamics — the true reverse process re-injects a little noise at every step.</p>
