<template>
  <section class="lecture" id="l5" ref="rootEl">
    ...
    <p>Each action is now credited only with what followed it. (Strictly, each term also inherits a \(\gamma^t\) weight from the discounted objective; standard practice — and every implementation, including ours below — drops it.) Notice \(\hat Q_t\) is a single-sample Monte-Carlo estimate of \(Q^{\pi}(s_t,a_t)\) — value functions are sneaking back in through the side door.</p>
    ...
    <p>The claim of §5.4 sounds too good: subtract a baseline and the gradient's <em>average</em> is unchanged (still unbiased), while its <em>variance</em> can plummet. See it directly. Below are ten sampled actions; each contributes a gradient term \(g_i=(R_i-b)\,\text{score}_i\). With \(b=0\), every return is positive, so every action is shoved upward with large, similar magnitude — the useful <em>differences</em> between actions are drowned out. Slide \(b\) and watch two things at once: the orange dot rides the variance curve down into its dip near the mean return, while the mean-gradient needle below conspicuously refuses to move — the noise collapses, the signal stays put, and that is why baselines are free.</p>
    <BaseWidget />

    <details class="dive"><summary>Going deeper: REINFORCE in ~25 lines — watch the absence of a model</summary><div class="dive-body">
      <p>The boxed equation of §5.2, as runnable-shaped pseudocode (PyTorch-flavored) — now with everything you've since added: reward-to-go and a crude baseline. The exercise: find the environment dynamics in this code. You can't — that's the theorem, made physical.</p>
...
for iteration in range(N):
    ...
    <p>Trust Region Policy Optimization (TRPO) formalizes this as maximizing a surrogate subject to a KL-divergence trust region — principled, but heavy (second-order). (KL divergence, if new: the standard measure of how different two distributions are, \(\mathrm{KL}(p\Vert q)=\mathbb E_{x\sim p}[\log p(x)-\log q(x)]\ge 0\), zero only when they match; asymmetric, so not a true distance. You already use it implicitly — cross-entropy = entropy + KL.) <strong>PPO</strong> (Proximal Policy Optimization) gets the same effect with a clipped first-order objective. Define the probability ratio against the data-collecting policy:</p>
    <p>$$r_t(\theta) = \frac{\pi_\theta(a_t|s_t)}{\pi_{\theta_{\text{old}}}(a_t|s_t)}$$</p>
    <p>Why a ratio? Two observations. First, samples from \(\pi_{\theta_{\text{old}}}\) can still estimate expectations under \(\pi_\theta\) if each sample is reweighted by how much likelier the new policy makes it — that weight is \(r_t\) (statisticians call this importance sampling), and it's what lets one batch of rollouts survive several gradient epochs. Second, the surrogate \(\mathbb E[r_t(\theta)\hat A_t]\) has exactly §5.4's gradient where it matters: \(\nabla_\theta r_t = r_t\nabla_\theta\log\pi_\theta\), and at \(\theta=\theta_{\text{old}}\), \(r_t=1\) — so its gradient there is \(\mathbb E[\nabla\log\pi\cdot\hat A]\). PPO clips that surrogate:</p>
    ...
    <p>DDPG and TD3 explore by bolting hand-tuned noise onto a deterministic actor — a schedule the objective knows nothing about, and the source of much of the brittleness just described. <strong>SAC</strong> (Soft Actor-Critic) — the modern default for real-robot RL — makes exploration part of the objective itself, by paying the policy for staying random:</p>
    <p>$$J(\pi) = \sum_t \mathbb E\big[\, r_t + \alpha\, \mathcal H\big(\pi(\cdot|s_t)\big)\big]$$</p>
    <p>Maximize reward <em>while staying as random as possible</em>. (\(\mathcal H(\pi(\cdot|s))=\mathbb E_{a\sim\pi}[-\log\pi(a|s)]\) — expected surprise: spread-out policy, high entropy. That definition is why the actor loss below reads \(\alpha\log\pi-Q\): per sampled action the bonus is \(-\alpha\log\pi\), negated because we minimize.) Effects: exploration is built into the objective (no external noise schedules), the policy hedges across all good actions instead of collapsing onto one (robustness), and the temperature \(\alpha\) is auto-tuned against a target entropy. The critic learns a "soft" Q with an entropy-augmented Bellman target, and the stochastic actor is trained by minimizing \(\mathbb E_{s}\big[\mathbb E_{a\sim\pi_\theta}[\alpha\log\pi_\theta(a|s) - Q_\phi(s,a)]\big]\) using the <strong>reparameterization trick</strong>: \(a = \tanh(\mu_\theta(s) + \sigma_\theta(s)\odot\varepsilon),\ \varepsilon\sim\mathcal N(0,I)\) — exactly the VAE move. (\(\odot\) = elementwise product.) The point: the randomness now lives entirely in \(\varepsilon\), which contains no \(\theta\) — given a draw of \(\varepsilon\), the action is an ordinary differentiable function of \(\theta\), so low-variance gradients flow from \(Q_\phi\) through \(a\) into \(\mu_\theta,\sigma_\theta\) by plain backprop. One notorious implementation wrinkle: because the action is squashed through \(\tanh\), the log-probability needs a change-of-variables correction \(\log\pi(a|s) = \log\mathcal N(u) - \sum_i \log\!\big(1-\tanh^2(u_i)\big)\) (with \(u=\mu+\sigma\odot\varepsilon\)) — omit that Jacobian term and the entropy bonus is silently wrong. HIL-SERL (Lecture 4's paper) and most sample-efficient hardware RL sit on this foundation.</p>
