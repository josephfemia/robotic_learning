<template>
  <section class="lecture" id="l2" ref="rootEl">
    ...
    <p>Before asking why this must converge, watch it converge. Below is the classic gridworld: a \(+1\) goal, a \(-1\) pit, one wall. Every cell starts believing it's worth \(0\). Press <strong>Step</strong> to apply the Bellman optimality update everywhere at once — each cell looks at its neighbors and keeps the best <em>(reward + discounted neighbor value)</em>. Watch value bleed outward from the goal, one ring per sweep, and watch the greedy policy arrows snap into a coherent plan the instant the numbers beneath them make sense. And notice the thing that needs explaining: the values don't oscillate or wander; every sweep visibly tightens them. That regularity is the theorem of the next paragraph, made visible.</p>
    <GridWidget />

    <p>Why did that outward propagation settle so obediently, from any initialization? Because the Bellman optimality operator \(\mathcal T\) is a <strong>\(\gamma\)-contraction</strong> in the sup-norm:</p>
