...
import { writeOn, focusPulse } from '../composables/useAnimate.js';
...
onMounted(() => {
  ...
  var prevDiverged = null; // track status flips to pulse only on change

  // Curves are distinguished by the legend below the plot (cyan = v(s)=w,
  // orange = v(s′)=2w). The old inline curve-end labels collided with each
  // other when both curves converged, and with the status word when v(s′)
  // shot up to the top-right on divergence — so they were dropped.
  function draw() {
    R.clr(svg);
    ...
    var cols = [R.C.cyan, R.C.orange];
    var paths = [];
    for (var i = 0; i < 2; i++) {
      var d = '';
      for (var t = 0; t < hist.length; t++) { d += (t ? 'L' : 'M') + X(t).toFixed(1) + ' ' + Y(hist[t][i]).toFixed(1); }
      var p = R.E('path', { d: d, fill: 'none', stroke: cols[i], 'stroke-width': 2.2 });
    ...
    var status = R.TX(x1, 16, diverged ? 'DIVERGING → ∞' : 'stable', { anchor: 'end', fill: diverged ? R.C.red : R.C.green, size: 13, weight: 700, base: 'hanging' });
    svg.appendChild(status);
    svg.appendChild(R.TX(W / 2, H - 12, 'training steps →', { fill: R.C.dim, size: 11 }));
    return { paths: paths, status: status, diverged: diverged };
  }

  // Redraw, write-on the two curves (left→right, like training proceeding),
  // and pulse the status word red the moment it flips into divergence.
  function redraw() {
    var r = draw();
    r.paths.forEach(function (p) { writeOn(p, { dur: 520 }); });
    if (r.diverged && prevDiverged === false) focusPulse(r.status, { color: R.C.red });
    prevDiverged = r.diverged;
  }

  var b1 = R.btn(ctr, 'Function approximation: OFF', null, function () { fa = !fa; b1.textContent = 'Function approximation: ' + (fa ? 'ON' : 'OFF'); b1.classList.toggle('primary', fa); redraw(); });
  var b2 = R.btn(ctr, 'Bootstrapping: OFF', null, function () { boot = !boot; b2.textContent = 'Bootstrapping: ' + (boot ? 'ON' : 'OFF'); b2.classList.toggle('primary', boot); redraw(); });
  var b3 = R.btn(ctr, 'Off-policy data: OFF', null, function () { off = !off; b3.textContent = 'Off-policy data: ' + (off ? 'ON' : 'OFF'); b3.classList.toggle('primary', off); redraw(); });
  ...
  R.legend(stage, [[R.C.cyan, 'v(s) = w'], [R.C.orange, 'v(s′) = 2w']]);
  var first = draw(); prevDiverged = first.diverged;
});
</script>
