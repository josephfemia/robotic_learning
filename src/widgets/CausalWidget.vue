...
onMounted(() => {
  ...
  // `disp` holds the currently-shown bar values so a toggle eases the bars
  // (height, %, colour) between states instead of snapping.
  var disp={train:trainingAccuracy(true),deploy:deploymentSuccess(true)};
  function barColTrain(v){return v>0.8?R.C.green:R.C.red;}
  function barColDeploy(v){return v>0.5?R.C.green:R.C.red;}
  ...
  function draw(){R.clr(svg);
   svg.appendChild(R.E('line',{x1:40,y1:H-72,x2:W-40,y2:H-72,stroke:R.C.axis,'stroke-width':1.2}));
   bar(130,150,disp.train,barColTrain(disp.train),'training accuracy','on logged demos');
   bar(420,150,disp.deploy,barColDeploy(disp.deploy),'closed-loop success','driving for real');
  ...
  function show(v){
   incl=v;
   var from={train:disp.train,deploy:disp.deploy};
   var to={train:trainingAccuracy(v),deploy:deploymentSuccess(v)};
   tween(420,{onStep(e){
     disp.train=from.train+(to.train-from.train)*e;
     disp.deploy=from.deploy+(to.deploy-from.deploy)*e;
     draw();
   },onDone(){disp.train=to.train;disp.deploy=to.deploy;draw();}});
  }
  R.btn(ctr,'Remove the brake-light feature',null,function(){show(false);});
  R.btn(ctr,'Add the brake-light feature','primary',function(){show(true);});
