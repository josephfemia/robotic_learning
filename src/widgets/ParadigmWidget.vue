...
onMounted(() => {
  ...
  // Selection clicks are discrete state changes, so their consequences ease:
  // newly revealed question rows and a new/changed result panel fade in via
  // tween() (instant under prefers-reduced-motion). Already-visible rows and
  // the clicked pill itself redraw in place — no motion on unchanged chrome.
  var prevVis = { model: false, signal: false, online: false };
  var prevLeafName = undefined;
  function draw() {
    R.clr(svg);
    var fadeEls = [];
    var vis = { model: false, signal: false, online: false };
    var qs = [
    ...
    for (var i = 0; i < qs.length; i++) {
      ...
      vis[Q.key] = true;
      var rowG = R.E('g', {});
      rowG.appendChild(R.TX(40, Q.y, Q.text, { anchor: 'start', fill: R.C.ink, size: 13, weight: 600, base: 'middle' }));
      var bx = 40;
      for (var j = 0; j < Q.opts.length; j++) {
        ...
        rowG.appendChild(g); bx += tw + 12;
      }
      svg.appendChild(rowG);
      if (!prevVis[Q.key]) fadeEls.push(rowG);
    }
    // result
    ...
    var resG = R.E('g', {});
    if (L) {
      resG.appendChild(R.E('rect', { x: 40, y: ry - 6, width: W - 70, height: 54, rx: 8, fill: 'rgba(47,203,126,0.10)', stroke: R.C.green, 'stroke-width': 1.5 }));
      resG.appendChild(R.TX(54, ry + 12, '→ ' + L.name, { anchor: 'start', fill: R.C.green, size: 14, weight: 700, base: 'middle' }));
      resG.appendChild(R.TX(54, ry + 32, L.why, { anchor: 'start', fill: R.C.ink, size: 11.5, base: 'middle' }));
      ...
      resG.appendChild(R.TX(54, ry + 18, 'Answer the questions above to route to a paradigm…', { anchor: 'start', fill: R.C.dim, size: 12.5, base: 'middle' }));
    }
    svg.appendChild(resG);
    var leafName = L ? L.name : null;
    if (prevLeafName !== undefined && leafName !== prevLeafName) fadeEls.push(resG);
    prevVis = vis;
    prevLeafName = leafName;
    if (fadeEls.length) {
      for (var f = 0; f < fadeEls.length; f++) fadeEls[f].setAttribute('opacity', 0);
      tween(320, { onStep: function (e) {
        for (var f = 0; f < fadeEls.length; f++) fadeEls[f].setAttribute('opacity', e);
      } });
    }
  }
  // First paint: mark everything visible as "already seen" so the initial
  // draw appears without motion (only user-driven changes ease).
  prevVis = { model: true, signal: false, online: false };
  draw();
});
</script>
