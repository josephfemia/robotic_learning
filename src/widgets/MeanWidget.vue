...
import { gaussian, msePrediction, MODE_LEFT, MODE_RIGHT, DEMO_SIGMA2, MSE_SIGMA2 } from '../logic/meanCollapse.js';
...
onMounted(() => {
  ...
  // Numeric core (gaussian, msePrediction, mode/variance constants) comes from
  // logic/meanCollapse.js (vitest-pinned), identical to the original's inline math.
  ...
  var dots=[];
  function sampleDots(){dots=[];for(var k=0;k<14;k++){var side=Math.random()<0.5?MODE_LEFT:MODE_RIGHT;dots.push(side+(Math.random()-0.5)*0.28);}}
  sampleDots();
  ...
  function draw(prog){
   ...
   R.clr(svg);var px=ax(msePrediction(MODE_LEFT,MODE_RIGHT));
   ...
   function hump(cx){var p='';for(var i=0;i<=44;i++){var a=-1+2*i/44,g=gaussian(a,cx,DEMO_SIGMA2),x=ax(a),y=axisY-g*64;p+=(i?' ':'')+x.toFixed(1)+','+y.toFixed(1);}svg.appendChild(R.E('polyline',{points:p,fill:'none',stroke:'rgba(54,197,208,0.85)','stroke-width':2}));}
   hump(MODE_LEFT);hump(MODE_RIGHT);
   svg.appendChild(R.TX(ax(MODE_LEFT),axisY-72,'"go left" demos',{fill:R.C.cyan,size:11}));
   svg.appendChild(R.TX(ax(MODE_RIGHT),axisY-72,'"go right" demos',{fill:R.C.cyan,size:11}));
   // Verdict text lives in a top header band (y=26), clear of the obstacle box.
   if(mode==='mse'){var p='';for(var i=0;i<=44;i++){var a=-1+2*i/44,g=gaussian(a,msePrediction(MODE_LEFT,MODE_RIGHT),MSE_SIGMA2),x=ax(a),y=axisY-g*64*prog;p+=(i?' ':'')+x.toFixed(1)+','+y.toFixed(1);}svg.appendChild(R.E('polyline',{points:p,fill:'none',stroke:R.C.red,'stroke-width':2.5,opacity:prog}));
