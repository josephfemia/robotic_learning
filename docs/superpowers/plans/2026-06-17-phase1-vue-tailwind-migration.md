...
- [ ] **Step 3: `index.html`** — app shell with the original's `<head>` CDN `<link>`s **verbatim** (KaTeX CSS, the exact Google Fonts URL for Archivo/Source Serif 4/IBM Plex Mono), KaTeX auto-render JS `<script defer src>`, `<title>` identical, `<div id="app">`, `<script type="module" src="/src/main.js">`.
...
- [ ] **Step 2:** `useAnimate.js` — port `animate(dur, step, done)` (the exact ease: `p<0.5?2*p*p:1-Math.pow(-2*p+2,2)/2`) as a composable that respects `prefers-reduced-motion` by jumping to the end state (matches original reduced-motion CSS intent). **Do not extend it** beyond the original in Phase 1 (that's Phase 2).
...
- [ ] **d. Build `<Widget>.vue`** — controls (sliders/buttons matching original ranges/labels/`fmt`), draw routine using `rllab.js` helpers and `useAnimate` where the original animated (only `pg`, `saycan`, and the few noted — others snap, preserve that). Same `id`, same `.lab-note` text, same legend.
